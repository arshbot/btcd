...
func ComputeTaprootOutputKey(internalKey *btcec.PublicKey,
	...
	// This routine only operates on x-only public keys where the the
	// public key always has an even y coordindate, so we'll re-parse it as
	// such.
	internalKey, _ = schnorr.ParsePubKey(
